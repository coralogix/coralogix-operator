...
var _ = Describe("Recording Rule Group Set", Ordered, func() {
	var (
	...
	BeforeAll(func() {
		...
		recordingRuleGroupSetClient = ClientsInstance.GetCoralogixClientSet().RecordingRuleGroups()
	})

	It("Should be created successfully", func(ctx context.Context) {
		By("Defining a RecordingRuleGroupSet resource")
		...
		By("Creating the RecordingRuleGroupSet resource in the cluster")
		...
		Eventually(func(g Gomega) error {
			err := crClient.Get(ctx, types.NamespacedName{Name: recordingRuleGroupSetName, Namespace: testNamespace}, fetchedRecordingRuleGroupSet)
		...
		Eventually(func() error {
			_, err := recordingRuleGroupSetClient.Get(ctx, &cxsdk.GetRuleGroupSetRequest{Id: recordingRuleGroupSetID})
			return err
		}, time.Minute, time.Second).Should(Succeed())
	})

	It("Should be updated successfully", func(ctx context.Context) {
		By("Patching the RecordingRuleGroupSet resource")
		...
		err := crClient.Patch(ctx, modifiedRecordingRuleGroupSet, client.MergeFrom(recordingRuleGroupSet))
		Expect(err).NotTo(HaveOccurred())

		By("Verifying RecordingRuleGroupSet is updated in Coralogix backend")
		Eventually(func(g Gomega) string {
			getRuleGroupRes, err := recordingRuleGroupSetClient.Get(ctx, &cxsdk.GetRuleGroupSetRequest{Id: recordingRuleGroupSetID})
			g.Expect(err).NotTo(HaveOccurred())
	...
	It("Should be deleted successfully", func(ctx context.Context) {
		By("Deleting the RecordingRuleGroupSet resource")
		err := crClient.Delete(ctx, recordingRuleGroupSet)
		Expect(err).NotTo(HaveOccurred())
