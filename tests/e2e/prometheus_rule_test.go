...
var _ = Describe("Prometheus Rule", Ordered, func() {
	var (
	...
	It("Should be created successfully", func(ctx context.Context) {
		By("Defining a PrometheusRule resource")
		promRule = &prometheus.PrometheusRule{
		...
		By("Creating the PrometheusRule resource in the cluster")
		Expect(crClient.Create(ctx, promRule)).To(Succeed())

		By("Verifying underlying Alert and RecordingRuleGroupSet resources were created in the cluster")
	...
	It("Should be updated successfully", func(ctx context.Context) {
		By("Patching the PrometheusRule resource")
		modifiedPromRule := promRule.DeepCopy()
		modifiedPromRule.Spec.Groups[0].Rules[0].Alert = newAlertName
		err := crClient.Patch(ctx, modifiedPromRule, client.MergeFrom(promRule))
		Expect(err).NotTo(HaveOccurred())

		By("Verifying underlying Alert resource was updated in the cluster")
		Eventually(func(g Gomega) error {
			fetchedAlert := &coralogixv1alpha1.Alert{}
	...
	It("Should be deleted successfully", func(ctx context.Context) {
		...
		err := crClient.Delete(ctx, promRule)
		Expect(err).NotTo(HaveOccurred())

		By("Verifying underlying Alert and RecordingRuleGroupSet resources were deleted from the cluster")
		fetchedAlert := &coralogixv1alpha1.Alert{}
