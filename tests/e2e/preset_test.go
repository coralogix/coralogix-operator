...
var _ = Describe("Preset", Ordered, func() {
		...
		preset = &coralogixv1alpha1.Preset{
			...
			Spec: coralogixv1alpha1.PresetSpec{
				...
				ConnectorType: &coralogixv1alpha1.PresetConnectorType{
					Slack: &coralogixv1alpha1.PresetConnectorTypeSlack{
						General: &coralogixv1alpha1.PresetConnectorTypeSlackGeneral{
							StructuredFields: &coralogixv1alpha1.PresetConnectorTypeSlackStructuredFields{
								Title:       ptr.To("Critical Alert"),
								Description: ptr.To("This is a critical alert notification."),
								Footer:      ptr.To("Generated by Coralogix"),
							},
						},
						Overrides: []coralogixv1alpha1.PresetConnectorTypeSlackOverride{
							{
								StructuredFields: &coralogixv1alpha1.PresetConnectorTypeSlackStructuredFields{
							...
							{
								RawFields: &coralogixv1alpha1.PresetConnectorTypeSlackRawFields{
	...
	It("should deny creation of Preset with two types", func(ctx context.Context) {
		preset.Spec.ConnectorType = &coralogixv1alpha1.PresetConnectorType{
			GenericHttps: &coralogixv1alpha1.PresetConnectorTypeGenericHttps{},
			Slack:        &coralogixv1alpha1.PresetConnectorTypeSlack{},
		}
	...
	It("should deny creation of Preset with invalid subtypes", func(ctx context.Context) {
		preset.Spec.ConnectorType = &coralogixv1alpha1.PresetConnectorType{
			Slack: &coralogixv1alpha1.PresetConnectorTypeSlack{
				Overrides: []coralogixv1alpha1.PresetConnectorTypeSlackOverride{
					{
						EntitySubType: "invalidSubType",
					},
				},
			},
		}
		err := crClient.Create(ctx, preset)
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("invalid entity subtypes"))
	})
})
