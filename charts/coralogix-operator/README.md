...
| coralogixOperator.labelSelector | object | `{}` | A comma-separated list of key=value labels to filter custom resources |
...
| coralogixOperator.namespaceSelector | object | `{}` | A list of namespaces to filter custom resources (by the namespaces' labels). |
