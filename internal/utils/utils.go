...
func GetGVKs(scheme *runtime.Scheme, notificationCenterEnabled bool) []schema.GroupVersionKind {
	result := []schema.GroupVersionKind{
		{Group: MonitoringAPIGroup, Version: V1APIVersion, Kind: PrometheusRuleKind},
	}

	result = append(result, GetGVKsInVersion(
		V1alpha1APIVersion,
		scheme,
...
func GetGVKsInVersion(version string, scheme *runtime.Scheme, notificationCenterEnabled bool) []schema.GroupVersionKind {
	...
	for kind := range knownTypes {
		...
		if kind == ConnectorKind || kind == PresetKind || kind == GlobalRouterKind {
