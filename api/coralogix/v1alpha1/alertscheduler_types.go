...
// AlertSchedulerSpec defines the desired state of AlertScheduler.
type AlertSchedulerSpec struct {
	...
	// Alert Scheduler filter. Only one of `metaLabels` or `alerts` can be set.
...
var (
	schemaToProtoScheduleOperation = map[string]cxsdk.ScheduleOperation{
		"active": cxsdk.ScheduleOperationActivate,
...
func extractAlertId(alert AlertRef, schedulerNamespace string) (string, error) {
	...
	a := &v1beta1.Alert{}
	err := config.GetConfig().Client.Get(context.Background(), client.ObjectKey{Name: alert.ResourceRef.Name, Namespace: namespace}, a)
	if err != nil {
		return "", err
	}
